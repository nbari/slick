...
NOOP_OUTPUT=$(HOME=/tmp/slick-loader-noop zsh -dfi -c 'PATH=/usr/local/bin source ./slick.zsh; print -r -- ${+functions[slick_prompt_preexec]}:${SLICK_PATH-unset}' 2>/dev/null)
...
[[ "$SLICK_PATH" == "$SLICK_BINARY" ]] || die "slick.plugin.zsh should preserve an explicit SLICK_PATH"
...
[[ "$SLICK_PATH" == "$SLICK_BINARY" ]] || die "load.zsh should prefer the local release binary"
